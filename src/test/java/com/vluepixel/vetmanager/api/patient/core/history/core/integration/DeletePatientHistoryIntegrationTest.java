...
public class DeletePatientHistoryIntegrationTest extends BaseIntegrationTest {
    ...
    void noUser_DeletePatientHistoryWithValidParams_Forbidden() throws Exception {
        mockMvc.perform(delete("/patient/{patient_id}/history/{id}", 1, VALID_UPDATE_PATIENT_HISTORY_REQUEST.getId()))
    ...
    void noUser_DeletePatientHistoryWithInvalidParams_ID_NotFound_Forbidden() throws Exception {
        mockMvc.perform(delete("/patient/{patient_id}/history/{id}", 1,
                INVALID_ID_NOT_FOUND_UPDATE_PATIENT_HISTORY_REQUEST.getId()))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message").value(MESSAGE_FORBIDDEN));
    }
    ...
    void noUser_DeletePatientHistoryWithInvalidParams_ID_Negative_Forbidden() throws Exception {
        mockMvc.perform(delete("/patient/{patient_id}/history/{id}", 1, -10))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message").value(MESSAGE_FORBIDDEN));
    }
    ...
    void noUser_DeletePatientHistoryWithInvalidParams_ID_NotNumber_Forbidden() throws Exception {
        mockMvc.perform(delete("/patient/{patient_id}/history/{id}", 1, "abcde"))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message").value(MESSAGE_FORBIDDEN));
    }
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(MESSAGE_NOT_FOUND.apply(
                                        INVALID_ID_NOT_FOUND_UPDATE_PATIENT_HISTORY_REQUEST.getId().toString())));
    }
